...
class BaseAviary_(gym.Env):
    ...
    def __init__(self,
                 ...
                 initial_xyzs=None,
                 initial_rpys=None,
                 initial_xyzs_preys=None,
                 initial_rpys_preys=None,
        ...
        self.NEIGHBOURHOOD_RADIUS_PREYS = neighbourhood_radius
        self.PREYS = preys
        #### Options ###############################################
        ...
        if self.GUI:
            ...
            if self.USER_DEBUG:
                ...
                self.INPUT_SWITCH_PREYS = p.addUserDebugParameter("Use GUI RPM", 9999, -1, 0, physicsClientId=self.CLIENT)
        else:
            #### Without debug GUI #####################################
        ...
        #### Set initial poses #####################################
        if initial_xyzs is None and initial_rpys_preys is None:
            ...
            self.INIT_XYZS_PREYS = np.vstack([np.array([x*4*self.L for x in range(self.PREYS)]), \
                                            np.array([y*4*self.L for y in range(self.PREYS)]), \
                                            np.ones(self.PREYS) * (self.COLLISION_H/2-self.COLLISION_Z_OFFSET+.1)]).transpose().reshape(self.PREYS, 3)
        elif np.array(initial_xyzs).shape == (self.NUM_DRONES,3) and np.array(initial_xyzs_preys).shape == (self.PREYS,3):
            self.INIT_XYZS = initial_xyzs
            self.INIT_XYZS_PREYS = initial_xyzs_preys
        else:
            # print(self.NUM_DRONES, self.PREYS)
            # print(initial_xyzs, initial_xyzs_preys)
            print("[ERROR] invalid initial_xyzs in BaseAviary.__init__(), try initial_xyzs.reshape(NUM_DRONES,3)")
        if initial_rpys is None and initial_rpys_preys is None:
            self.INIT_RPYS = np.zeros((self.NUM_DRONES, 3))
            self.INIT_RPYS_PREYS = np.zeros((self.PREYS, 3))
        elif np.array(initial_rpys).shape == (self.NUM_DRONES, 3) and np.array(initial_rpys_preys).shape == (self.PREYS, 3):
            self.INIT_RPYS = initial_rpys
            self.INIT_RPYS_PREYS = initial_rpys_preys
        else:
            print("[ERROR] invalid initial_rpys in BaseAviary.__init__(), try initial_rpys.reshape(NUM_DRONES,3)")
        #### Create action and observation spaces ##################
        self.action_space = self._actionSpace()
        self.action_space_preys = self._actionSpacePreys()
        self.observation_space = self._observationSpace()
        self.observation_space_preys = self._observationSpacePreys()
        #### Housekeeping ##########################################
        self._housekeeping()
        #### Update and store the drones kinematic information #####
        self._updateAndStoreKinematicInformation()
        self._updateAndStoreKinematicInformationPreys()
    ...
    def reset(self,
        ...
        initial_obs_preys = self._computeObsPreys()
        initial_info = self._computeInfo()
        return initial_obs, initial_obs_preys, initial_info
    ...
    def step(self,
        ...
        if self.RECORD and not self.GUI and self.step_counter%self.CAPTURE_FREQ == 0:
            ...
            if self.VISION_ATTR:
                for i in range(self.NUM_DRONES):
                    for j in range(self.PREYS):
                        self.rgb[i], self.dep[i], self.seg[i] = self._getDroneImages(i) # this wont WORK!!!
                        self.rgb[j], self.dep[j], self.seg[j] = self._getDroneImagesPreys(j)
                        ...
                        self._exportImage(img_type=ImageType.RGB, # ImageType.BW, ImageType.DEP, ImageType.SEG
                                        img_input=self.rgb[j],
                                        path=self.ONBOARD_IMG_PATH+"/drone_"+str(j)+"/",
                                        frame_num=int(self.step_counter/self.IMG_CAPTURE_FREQ)
                                        )
        #### Read the GUI's input parameters #######################
        if self.GUI and self.USER_DEBUG:
            current_input_switch = p.readUserDebugParameter(self.INPUT_SWITCH, physicsClientId=self.CLIENT)
            current_input_switch_preys = p.readUserDebugParameter(self.INPUT_SWITCH_PREYS, physicsClientId=self.CLIENT)
            if current_input_switch > self.last_input_switch and current_input_switch_preys > self.last_input_switch_preys:
                self.last_input_switch = current_input_switch
                self.last_input_switch_preys = current_input_switch_preys
                self.USE_GUI_RPM = True if self.USE_GUI_RPM == False else False
        if self.USE_GUI_RPM:
            for i in range(4):
                self.gui_input[i] = p.readUserDebugParameter(int(self.SLIDERS[i]), physicsClientId=self.CLIENT)
            clipped_action = np.tile(self.gui_input, (self.NUM_DRONES, 1))
            clipped_action_preys = np.tile(self.gui_input, (self.PREYS, 1))
            if self.step_counter%(self.PYB_FREQ/2) == 0:
                ...
                self.GUI_INPUT_TEXT_PREYS = [p.addUserDebugText("Using GUI RPM",
                                                            textPosition=[0, 0, 0],
                                                            textColorRGB=[1, 0, 0],
                                                            lifeTime=1,
                                                            textSize=2,
                                                            parentObjectUniqueId=self.DRONE_IDS_PREYS[i],
                                                            parentLinkIndex=-1,
                                                            replaceItemUniqueId=int(self.GUI_INPUT_TEXT_PREYS[i]),
                                                            physicsClientId=self.CLIENT
                                                            ) for i in range(self.PREYS)]
        #### Save, preprocess, and clip the action to the max. RPM #
        else:
            clipped_action = np.reshape(self._preprocessAction(action), (self.NUM_DRONES, 4))
            clipped_action_preys = np.reshape(self._preprocessActionPreys(action_preys), (self.PREYS, 4))
        ...
        for _ in range(self.PYB_STEPS_PER_CTRL):
            ...
            for i, j in zip(range(self.NUM_DRONES), range(self.PREYS)):
                # for j in range (self.PREYS):
                    if self.PHYSICS == Physics.PYB:
                        self._physics(clipped_action[i, :], i)
                        self._physicsPrey(clipped_action_preys[j, :], j)
                    elif self.PHYSICS == Physics.DYN:
                        self._dynamics(clipped_action[i, :], i)
                        self._dynamicsPrey(clipped_action_preys[j, :], j)
                    elif self.PHYSICS == Physics.PYB_GND:
                        self._physics(clipped_action[i, :], i)
                        self._groundEffect(clipped_action[i, :], i)
                        self._physicsPrey(clipped_action_preys[j, :], j)
                        self._groundEffectPrey(clipped_action_preys[j, :], j)
                    elif self.PHYSICS == Physics.PYB_DRAG:
                        self._physics(clipped_action[i, :], i)
                        self._drag(self.last_clipped_action[i, :], i)
                        self._physicsPrey(clipped_action_preys[j, :], j)
                        self._dragPrey(self.last_clipped_action_preys[j, :], j)
                    elif self.PHYSICS == Physics.PYB_DW:
                        self._physics(clipped_action[i, :], i)
                        self._downwash(i)
                        self._physicsPrey(clipped_action_preys[j, :], j)
                        self._downwashPrey(j)
            ...
            self.last_clipped_action_preys = clipped_action_preys
        #### Update and store the drones kinematic information #####
        self._updateAndStoreKinematicInformation()
        self._updateAndStoreKinematicInformationPreys()
        #### Prepare the return values #############################
        obs = self._computeObs()
        obs_preys = self._computeObsPreys()
        # 
        reward = self._computeReward()
        reward_preys = self._computeReward()
        # 
        terminated = self._computeTerminated()
        terminated_preys = self._computeTerminated()
        # 
        truncated = self._computeTruncated()
        truncated_preys = self._computeTruncated()
        # 
        info = self._computeInfo()
        info_preys = self._computeInfo()
        #
        #### Advance the step counter ##############################
        self.step_counter = self.step_counter + (1 * self.PYB_STEPS_PER_CTRL)
        return obs, obs_preys, reward, reward_preys, terminated, terminated_preys, truncated, truncated_preys, info, info_preys
    ...
    #     if self.GUI and self.USER_DEBUG:
    #         current_input_switch = p.readUserDebugParameter(self.INPUT_SWITCH, physicsClientId=self.CLIENT)
    #         if current_input_switch > self.last_input_switch_preys:
    #             self.last_input_switch_preys = current_input_switch
    #             self.USE_GUI_RPM = True if self.USE_GUI_RPM == False else False
    #     if self.USE_GUI_RPM:
    #         for i in range(4):
    #             self.gui_input[i] = p.readUserDebugParameter(int(self.SLIDERS[i]), physicsClientId=self.CLIENT)
    ...
    #     #### Save, preprocess, and clip the action to the max. RPM #
    #     else:
    #         clipped_action_preys = np.reshape(self._preprocessActionPreys(action), (self.PREYS, 4))
    #     #### Repeat for as many as the aggregate physics steps #####
    #     for _ in range(self.PYB_STEPS_PER_CTRL):
    #         #### Update and store the drones kinematic info for certain
    #         #### Between aggregate steps for certain types of update ###
    ...
    #         for i in range (self.PREYS):
    #             if self.PHYSICS == Physics.PYB:
    #                 self._physicsPrey(clipped_action_preys[i, :], i)
    #             elif self.PHYSICS == Physics.DYN:
    #                 self._dynamicsPrey(clipped_action_preys[i, :], i)
    #             elif self.PHYSICS == Physics.PYB_GND:
    #                 self._physicsPrey(clipped_action_preys[i, :], i)
    #                 self._groundEffect(clipped_action_preys[i, :], i)
    #             elif self.PHYSICS == Physics.PYB_DRAG:
    #                 self._physicsPrey(clipped_action_preys[i, :], i)
    #                 self._drag(self.last_clipped_action_preys[i, :], i)
    #             elif self.PHYSICS == Physics.PYB_DW:
    #                 self._physicsPrey(clipped_action_preys[i, :], i)
    #                 self._downwashPrey(i)
    #             elif self.PHYSICS == Physics.PYB_GND_DRAG_DW:
    #                 self._physicsPrey(clipped_action_preys[i, :], i)
    #                 self._groundEffectPrey(clipped_action_preys[i, :], i)
    #                 self._dragPrey(self.last_clipped_action_preys[i, :], i)
    #                 self._downwashPrey(i)
    #         #### PyBullet computes the new state, unless Physics.DYN ###
    #         if self.PHYSICS != Physics.DYN:
    #             p.stepSimulation(physicsClientId=self.CLIENT)
    ...
    def render(self,
        ...
        for i in range (self.PREYS):
            print("[INFO] BaseAviary.render() ——— prey {:d}".format(i),
                  "——— x {:+06.2f}, y {:+06.2f}, z {:+06.2f}".format(self.pos_preys[i, 0], self.pos_preys[i, 1], self.pos_preys[i, 2]),
                  "——— velocity {:+06.2f}, {:+06.2f}, {:+06.2f}".format(self.vel_preys[i, 0], self.vel_preys[i, 1], self.vel_preys[i, 2]),
                  "——— roll {:+06.2f}, pitch {:+06.2f}, yaw {:+06.2f}".format(self.rpy_preys[i, 0]*self.RAD2DEG, self.rpy_preys[i, 1]*self.RAD2DEG, self.rpy_preys[i, 2]*self.RAD2DEG),
                  "——— angular velocity {:+06.4f}, {:+06.4f}, {:+06.4f} ——— ".format(self.ang_v_preys[i, 0], self.ang_v_preys[i, 1], self.ang_v_preys[i, 2]))
    ...
    def _housekeeping(self):
        ...
        # preys
        self.X_AX_PREYS = -1*np.ones(self.PREYS)
        self.Y_AX_PREYS = -1*np.ones(self.PREYS)
        self.Z_AX_PREYS = -1*np.ones(self.PREYS)
        self.GUI_INPUT_TEXT_PREYS = -1*np.ones(self.PREYS)
        # self.step_counter = 0
        self.USE_GUI_RPM=False
        self.last_input_switch = 0
        self.last_input_switch_preys = 0
        self.last_clipped_action = np.zeros((self.NUM_DRONES, 4))
        self.last_clipped_action_preys = np.zeros((self.PREYS, 4))
        ...
        self.pos_preys = np.zeros((self.PREYS, 3))
        self.quat_preys = np.zeros((self.PREYS, 4))
        self.rpy_preys = np.zeros((self.PREYS, 3))
        self.vel_preys = np.zeros((self.PREYS, 3))
        self.ang_v_preys = np.zeros((self.PREYS, 3))

        if self.PHYSICS == Physics.DYN:
            self.rpy_rates = np.zeros((self.NUM_DRONES, 3))
            self.rpy_rates_preys = np.zeros((self.PREYS, 3))
        ...
        self.DRONE_IDS_PREYS = np.array([p.loadURDF(pkg_resources.resource_filename('gym_pybullet_drones', 'assets/'+self.URDF),
                                                    self.INIT_XYZS_PREYS[i,:],
                                                    p.getQuaternionFromEuler(self.INIT_RPYS_PREYS[i,:]),
                                                    flags = p.URDF_USE_INERTIA_FROM_FILE,
                                                    physicsClientId=self.CLIENT
                                                    ) for i in range(self.PREYS)])
    ...
    def _updateAndStoreKinematicInformationPreys(self):
        for i in range (self.PREYS):
            self.pos_preys[i], self.quat_preys[i] = p.getBasePositionAndOrientation(self.DRONE_IDS_PREYS[i], physicsClientId=self.CLIENT)
            self.rpy_preys[i] = p.getEulerFromQuaternion(self.quat_preys[i])
            self.vel_preys[i], self.ang_v_preys[i] = p.getBaseVelocity(self.DRONE_IDS_PREYS[i], physicsClientId=self.CLIENT)
    ...
    def _getDroneStateVectorPreys(self,
                             nth_drone
                             ):
        """Returns the state vector of the n-th drone.
        ...
        """
        state = np.hstack([self.pos_preys[nth_drone, :], self.quat_preys[nth_drone, :], self.rpy_preys[nth_drone, :],
                            self.vel_preys[nth_drone, :], self.ang_v_preys[nth_drone, :], self.last_clipped_action_preys[nth_drone, :]])
        return state.reshape(20,)
    ...
    def _getDroneImagesPreys(self,
        ...
        if self.IMG_RES is None:
            print("[ERROR] in BaseAviary._getDroneImages(), remember to set self.IMG_RES to np.array([width, height])")
            exit()
        rot_mat = np.array(p.getMatrixFromQuaternion(self.quat_preys[nth_drone, :])).reshape(3, 3)
        #### Set target point, camera view and projection matrices #
        target = np.dot(rot_mat,np.array([1000, 0, 0])) + np.array(self.pos_preys[nth_drone, :])
        DRONE_CAM_VIEW = p.computeViewMatrix(cameraEyePosition=self.pos_preys[nth_drone, :]+np.array([0, 0, self.L]),
                                             cameraTargetPosition=target,
                                             cameraUpVector=[0, 0, 1],
                                             physicsClientId=self.CLIENT
                                             )
    ...
    def _getAdjacencyMatrixPreys(self):
        """Computes the adjacency matrix of a multi-drone system.
        ...
        """
        adjacency_mat = np.identity(self.PREYS)
        for i in range(self.PREYS-1):
            for j in range(self.PREYS-i-1):
                if np.linalg.norm(self.pos_preys[i, :]-self.pos_preys[j+i+1, :]) < self.NEIGHBOURHOOD_RADIUS_PREYS:
                    adjacency_mat[i, j+i+1] = adjacency_mat[j+i+1, i] = 1
        return adjacency_mat
    ...
    def _physicsPrey(self,
        ...
        z_torque = (-torques[0] + torques[1] - torques[2] + torques[3])
        for i in range(4):
            p.applyExternalForce(self.DRONE_IDS_PREYS[nth_drone],
                                 i,
                                 forceObj=[0, 0, forces[i]],
                                 posObj=[0, 0, 0],
                                 flags=p.LINK_FRAME,
        ...
        p.applyExternalTorque(self.DRONE_IDS_PREYS[nth_drone],
                              4,
                              torqueObj=[0, 0, z_torque],
                              flags=p.LINK_FRAME,
                              physicsClientId=self.CLIENT
    ...
    def _groundEffectPrey(self,
        ...
        """
        #### Kin. info of all links (propellers and center of mass)
        link_states = p.getLinkStates(self.DRONE_IDS_PREYS[nth_drone],
                                        linkIndices=[0, 1, 2, 3, 4],
                                        computeLinkVelocity=1,
                                        computeForwardKinematics=1,
                                        physicsClientId=self.CLIENT
    ...
    def _dragPrey(self,
        ...
        """
        #### Rotation matrix of the base ###########################
        base_rot = np.array(p.getMatrixFromQuaternion(self.quat_preys[nth_drone, :])).reshape(3, 3)
        #### Simple draft model applied to the base/center of mass #
        drag_factors = -1 * self.DRAG_COEFF * np.sum(np.array(2*np.pi*rpm/60))
        drag = np.dot(base_rot.T, drag_factors*np.array(self.vel_preys[nth_drone, :]))
        p.applyExternalForce(self.DRONE_IDS_PREYS[nth_drone],
                             4,
                             forceObj=drag,
                             posObj=[0, 0, 0],
    ...
    def _downwashPrey(self,
        ...
        """
        for i in range(self.PREYS):
            delta_z = self.pos_preys[i, 2] - self.pos_preys[nth_drone, 2]
            delta_xy = np.linalg.norm(np.array(self.pos_preys[i, 0:2]) - np.array(self.pos_preys[nth_drone, 0:2]))
            if delta_z > 0 and delta_xy < 10: # Ignore drones more than 10 meters away
                alpha = self.DW_COEFF_1 * (self.PROP_RADIUS/(4*delta_z))**2
                beta = self.DW_COEFF_2 * delta_z + self.DW_COEFF_3
                downwash = [0, 0, -alpha * np.exp(-.5*(delta_xy/beta)**2)]
    ...
    def _dynamicsPrey(self,
        ...
        """
        #### Current state #########################################
        pos = self.pos_preys[nth_drone,:]
        quat = self.quat_preys[nth_drone,:]
        vel = self.vel_preys[nth_drone,:]
        rpy_rates = self.rpy_rates_preys[nth_drone,:]
        rotation = np.array(p.getMatrixFromQuaternion(quat)).reshape(3, 3)
        #### Compute forces and torques ############################
        forces = np.array(rpm**2) * self.KF
        thrust = np.array([0, 0, np.sum(forces)])
        ...
        quat = self._integrateQ(quat, rpy_rates, self.PYB_TIMESTEP)
        #### Set PyBullet's state ##################################
        p.resetBasePositionAndOrientation(self.DRONE_IDS_PREYS[nth_drone],
                                          pos,
                                          quat,
                                          physicsClientId=self.CLIENT
                                          )
        #### Note: the base's velocity only stored and not used ####
        p.resetBaseVelocity(self.DRONE_IDS_PREYS[nth_drone],
                            vel,
                            np.dot(rotation, rpy_rates),
                            physicsClientId=self.CLIENT
                            )
        #### Store the roll, pitch, yaw rates for the next step ####
        self.rpy_rates_preys[nth_drone,:] = rpy_rates
    ...
    def _showDroneLocalAxesPreys(self,
                            nth_drone
                            ):
        """Draws the local frame of the n-th drone in PyBullet's GUI.
        ...
        if self.GUI:
            AXIS_LENGTH = 2*self.L
            self.X_AX_PREYS[nth_drone] = p.addUserDebugLine(lineFromXYZ=[0, 0, 0],
                                                      lineToXYZ=[AXIS_LENGTH, 0, 0],
                                                      lineColorRGB=[1, 0, 0],
                                                      parentObjectUniqueId=self.DRONE_IDS_PREYS[nth_drone],
                                                      parentLinkIndex=-1,
                                                      replaceItemUniqueId=int(self.X_AX_PREYS[nth_drone]),
                                                      physicsClientId=self.CLIENT
                                                      )
            self.Y_AX_PREYS[nth_drone] = p.addUserDebugLine(lineFromXYZ=[0, 0, 0],
                                                      lineToXYZ=[0, AXIS_LENGTH, 0],
                                                      lineColorRGB=[0, 1, 0],
                                                      parentObjectUniqueId=self.DRONE_IDS_PREYS[nth_drone],
                                                      parentLinkIndex=-1,
                                                      replaceItemUniqueId=int(self.Y_AX_PREYS[nth_drone]),
                                                      physicsClientId=self.CLIENT
                                                      )
            self.Z_AX_PREYS[nth_drone] = p.addUserDebugLine(lineFromXYZ=[0, 0, 0],
                                                      lineToXYZ=[0, 0, AXIS_LENGTH],
                                                      lineColorRGB=[0, 0, 1],
                                                      parentObjectUniqueId=self.DRONE_IDS_PREYS[nth_drone],
                                                      parentLinkIndex=-1,
                                                      replaceItemUniqueId=int(self.Z_AX_PREYS[nth_drone]),
                                                      physicsClientId=self.CLIENT
                                                      )
    ################################################################################
